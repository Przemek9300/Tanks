...
class GhostTank(TankBase):
    ...
    def changeImage(self):
        if self.state == self.states[0]:
            self.image = pygame.image.load("img/GhostUP.png")
        if self.state == self.states[1]:
            self.image = pygame.image.load("img/GhostLEFT.png")
        if self.state == self.states[2]:
            self.image = pygame.image.load("img/GhostRIGHT.png")
        if self.state == self.states[3]:
            self.image = pygame.image.load("img/GhostDOWN.png")
    def changeDir(self):
    ...
    def update(self):
        ...
        for block in block_hit_list:
            ...
            # the item we hit
            if not isinstance(block, BushBlock) and not isinstance(block, BrickBlock):
                if self.move_x > 0:
                    self.changeDir()
